...
describe BRDocuments::IE::PI do
  ...
  it 'pretty formats an IE number' do
    expect(described_class.pretty("193013835")).to eq("19301383-5")
  end
  ...
  it 'remove an IE number formatting' do
    expect(described_class.strip('19301383-5')).to eq('193013835')
  end

  it 'must validate correctly valid IE numbers' do
    expect(described_class.valid?('19301383-5')).to be_truthy
    expect(described_class.valid?('19-3013835')).to be_truthy
    expect(described_class.valid?('193013835')).to be_truthy
  end

  it 'is invalid with wrong digits number' do
    expect(described_class.invalid?('19301383-51')).to be_truthy
  end

  it 'must calculate verify digits' do
    expected = [5]

    expect(described_class.calculate_verify_digits("19301383-5")).to eq(expected)
    expect(described_class.calculate_verify_digits("19301383")).to eq(expected)
  end
